...
class PostContentList extends StatelessWidget {
  final ForumPostViewModel viewModel;
  const PostContentList({super.key, required this.viewModel});
  ...
  Widget _buildRecruitmentCardList(BuildContext context) {
    // 1. 首次加载的 loading
    if (viewModel.isLoading && viewModel.forumPostInfoList.isEmpty) {
      return Padding(
    ...
    // 2. 空状态
    if (viewModel.forumPostInfoList.isEmpty) {
    ...
    return Column(
      children: [
        ...
        ListView.separated(
          ...
          itemCount: viewModel.forumPostInfoList.length,
          separatorBuilder: (context, index) => 16.verticalSpace,
          itemBuilder: (context, index) {
            return ForumPostCard(
              postInfo: viewModel.forumPostInfoList[index],
            );
          },
        ),
